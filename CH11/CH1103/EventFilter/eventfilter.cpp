...
bool EventFilter::eventFilter(QObject *watched, QEvent *event)
{
	...
	if (watched == label1 || watched == label2 || watched == label3) {
		auto label = static_cast<QLabel*>(watched);
		auto text = [this, watched]()->std::pair<QString, QImage&> {
			if (watched == label1) { return std::make_pair(QString("左边"), std::ref(Image1)); }
			else if (watched == label2) { return std::make_pair(QString("中间"), std::ref(Image2)); }
			else if (watched == label3) { return std::make_pair(QString("右边"), std::ref(Image3)); }
			else { return std::make_pair(QString(), std::ref(Image1)); }
		}();
		if (event->type() == QEvent::MouseButtonPress) {
			QMouseEvent *mouseEvent = (QMouseEvent *)event;
			if (mouseEvent->buttons()&Qt::LeftButton) {
				stateLabel->setText(tr("左键按下") + text.first + tr("图片"));
			} else if (mouseEvent->buttons()&Qt::MidButton) {
				stateLabel->setText(tr("中键按下") + text.first + tr("图片"));
			} else if (mouseEvent->buttons()&Qt::RightButton) {
				stateLabel->setText(tr("右键按下") + text.first + tr("图片"));
			}

			QMatrix matrix;
			matrix.scale(1.8, 1.8);
			QImage tmpImg = text.second.transformed(matrix);
			label->setPixmap(QPixmap::fromImage(tmpImg));
		}
		if (event->type() == QEvent::MouseButtonRelease) {
			stateLabel->setText(tr("鼠标释放") + text.first + tr("图片"));
			label->setPixmap(QPixmap::fromImage(text.second));
		}
	}
